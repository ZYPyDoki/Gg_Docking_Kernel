...
// 优化：增加 PAGE_SIZE 限制，防止因合并块过大导致驱动读取失败
...
long syscall(long number, ...) {
    ...
    // 严格检查：匹配 lib5.cpp 的逻辑，如果参数不合法直接返回错误
    ...
    // 使用volatile读取driver指针，强制从内存读取最新值，防止CPU缓存导致的空指针竞态
    auto volatile driver_ptr = driver;
    if (unlikely(driver_ptr == nullptr)) {
        errno = EIO;
        LOGE("Driver error: null pointer");
        return -1;
    }
    ...
    for (unsigned long i = 0; i < liovcnt; ++i) {
        ...
        // 关键修复：增加大小检查 (merge_len + len) <= page_size
