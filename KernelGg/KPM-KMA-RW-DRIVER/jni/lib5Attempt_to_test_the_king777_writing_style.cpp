...
#define HEADER_CHOICE 1 // 改变这个数字来决定你要编译对接什么驱动来作为头文件

#if HEADER_CHOICE == 0 // 判断如果输入数字0表示QXandOX12.8+完美驱动(proc方案)
    #include "driverQXOX12_8.h"
#elif HEADER_CHOICE == 1 // 判断如果输入数字1表示KMPandKPM内核模块驱动(maybe硬件读取)
    #include "KPMdriver.h"
#elif HEADER_CHOICE == 2 // 判断如果输入数字2表示GT1.5和1.6和1.7以及RT的dev(dev方案)
    #include "driverGT1_567RTdev.h"
#elif HEADER_CHOICE == 3 // 判断如果输入数字3表示GT2.1和2.2以及RT的hook(hook方案)
    #include "driverGT2_12RThook.h"
#elif HEADER_CHOICE == 4 // 判断如果输入数字4表示Dit的netlink方案和DitPro的hooksyscall方案
    #include "driverDitNetlink_Prohook.h"
#else
    #error "Invalid header choice"
#endif
...
#if (SAFE_OFF == 1)
#define KMA_READ(a, b, s) driver->read_safe(a, b, s) // 硬件读取
#else
#define KMA_READ(a, b, s) driver->read(a, b, s)     // 内核读取
#endif
...
static long vm_readv_custom(int pid, const iovec* local_iov, unsigned long liovcnt, const iovec* remote_iov, unsigned long riovcnt) {
    ...
    if (!driver->gid) {
        LOGE("vm_readv_custom: Driver GID not initialized");
        return -1;
    }
    ...
    // 优化2: SELinux单次修复
    static bool selinux_fixed = false;
    if (!selinux_fixed) {
        if (set_selinux_enforce("/sys/fs/selinux/enforce") == -1) {
            LOGE("Failed to enforce SELinux");
        }
        selinux_fixed = true;
    }
    ...
    for (unsigned long i = 0; i < liovcnt; ++i) {
        ...
        if (local_iov[i].iov_len <= PAGE_SIZE) {
            ...
            // 优化4: library2的分页读取逻辑，并使用最合理的 unlikely
            uintptr_t address_split = (uintptr_t)remote_iov[i].iov_base;
            char *buf = (char*)local_iov[i].iov_base;
            size_t size = local_iov[i].iov_len;

            for (size_t s = size; s > 0; s -= PAGE_SIZE) {
                // 使用更合理的 unlikely：读取不足一页是小概率事件
                if (unlikely(s < PAGE_SIZE)) {
                    KMA_READ(address_split, buf, s);
                } else {
                    KMA_READ(address_split, buf, PAGE_SIZE);
                    address_split += PAGE_SIZE;
                    buf += PAGE_SIZE;
                }
            }
        }
