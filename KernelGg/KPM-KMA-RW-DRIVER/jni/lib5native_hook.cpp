...
static std::unique_ptr<Driver> driver; // KMA当前版本 620.5
...
long syscall(long number, ...) {
    ...
    Driver* volatile driver_raw = driver.get();
    if (unlikely(driver_raw == nullptr)) {
        errno = EIO;
        LOGE("Driver error: null pointer");
        return -1;
    }
    
    if (!likely(driver->gid)) {
        errno = EIO;
        LOGE("Driver error: invalid gid");
        return -1;
    }
